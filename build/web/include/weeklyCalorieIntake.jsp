...
<%  WeeklyCalorieDA weeklyCalorieDA = new WeeklyCalorieDA();
    ...
    long ms = sqlStartDate.getTime();
    for(int p = 0;p<1;p++){
        ms += 86400000;
    }
    Date date2 = new Date(ms);
    
    ms = sqlStartDate.getTime();
    for(int p = 0;p<2;p++){
        ms += 86400000;
    }
    Date date3 = new Date(ms);
    
    ms = sqlStartDate.getTime();
    for(int p = 0;p<3;p++){
        ms += 86400000;
    }
    Date date4 = new Date(ms);
    
    ms = sqlStartDate.getTime();
    for(int p = 0;p<4;p++){
        ms += 86400000;
    }
    Date date5 = new Date(ms);
    
    ms = sqlStartDate.getTime();
    for(int p = 0;p<5;p++){
        ms += 86400000;
    }
    Date date6 = new Date(ms);

%>

<%@include file="../include/weeklyCalorieChart.jsp" %>
